...
class HardwareManager:
    ...
    def _find_zebra_printer(self):
        ...
        try:
            ...
            if default_p:
                ...
                print(
                    "⚠️ Alerta: Usando impresora predeterminada de Windows: "
                    f"{self.printer_name}"
                )
        except Exception as e:
        ...
        try:
            ...
            try:
                hJob = win32print.StartDocPrinter(
                    hPrinter, 1, ("Etiqueta TIF", None, "RAW")
                )
                try:
            ...
            return (
                False,
                f"Fallo al abrir impresora '{self.printer_name}': {str(e)}",
            )
    ...
    def print_ticket(self, pieza_data, caja_data, canal_data, producto_data):
        ...
        # CAMBIO CONGELADO: SOLO NUMERO DE CAJA (SIN CONSECUTIVO)
        caja_visual = f"{caja_data['numero_caja']}"

        siniiga_last4 = (
            siniiga_display[-4:]
            if len(siniiga_display) >= 4
            else siniiga_display.zfill(4)
        )
        caja_padded = str(caja_data["numero_caja"]).zfill(2)
        peso_padded = int(peso * 100)
        codigo_barras = (
            f"{lote_dia}_{siniiga_last4}{caja_padded}{codigo}{peso_padded:04d}"
        )
        ...
        zpl += (
            "^FO295,360^A0R,20,20^FDFECHA:^FS^FO295,480^A0R,25,25^FD"
            + fecha_actual
            + "^FS"
        )
        zpl += (
            "^FO265,360^A0R,20,20^FDESPECIE:^FS^FO265,480^A0R,22,22^FD"
            + especie
            + "^FS"
        )
        zpl += (
            "^FO235,360^A0R,20,20^FDLOTE:^FS^FO235,480^A0R,30,30^FD"
            + lote_dia
            + "^FS"
        )
        zpl += (
            "^FO205,360^A0R,20,20^FDSINIIGA:^FS^FO205,480^A0R,22,22^FD"
            + siniiga_display
            + "^FS"
        )

        # CAMBIO CONGELADO: "CAJA: <numero_caja>"
        zpl += "^FO175,360^A0R,20,20^FDCAJA:^FS^FO170,480^A0R,30,30^FD"
        zpl += caja_visual + "^FS"
